...
input_data = sio.loadmat("Data_Files/TX_Encoded1.mat")
...
input = torch.cat(
    [
        input_arr,
        torch.zeros((args.batch_size, 1, 2 * args.block_len)) + 0.5,
    ],
    dim=2,
)
...
sio.savemat("Data_Files/TX_Modulated1.mat", output)

hidden = {"hidden": np.array(hidden_state)}
sio.savemat("Data_Files/TX_Hidden1.mat", hidden)
