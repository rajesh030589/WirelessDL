function TX_Feedback_Encoder(num, scale)
    clc;
    ...
    elseif num == 3
        ...
        Z1_Input = open('Feedback_Files/Z1_Output.mat');
        Z1_Input = Z1_Input.ZL;
        Z2_Input = open('Feedback_Files/Z2_Output.mat');
        Z2_Input = Z2_Input.ZL;
    end

    C_Output = zeros(total_msg_symbols, no_of_frames);
    CX_Output = zeros(total_msg_symbols, no_of_frames);
    ...
    for n_frame = 1:no_of_frames
        ...
        if strcmp(mod_type, "NN")
            ...
            if num == 2
                encoder_data = cat(2, Bit_Input(:, n_frame), Z1_Input(:, n_frame));
            elseif num == 3
                encoder_data = cat(2, Bit_Input(:, n_frame), Z1_Input(:, n_frame), Z2_Input(:, n_frame));
            end
        ...
        CX_Output(:, n_frame) = tx_gain * mod_symbols;
    ...
    save(strcat('Feedback_Files/C', num2str(num), '_Output.mat'), 'C_Output');
    save(strcat('Feedback_Files/CX', num2str(num), '_Output.mat'), 'CX_Output');
