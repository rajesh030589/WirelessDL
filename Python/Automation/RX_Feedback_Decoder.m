function RX_Feedback_Decoder(num)
    clc;
    ...
    try
        ...
        YH = open(strcat('Feedback_Files/YH', num2str(num), '_Output.mat'));
        YH = YH.YH;

        YEQ = open(strcat('Feedback_Files/YEQ', num2str(num), '_Output.mat'));
        YEQ = YEQ.YEQ;

        YUEQ = open(strcat('Feedback_Files/YUEQ', num2str(num), '_Output.mat'));
        YUEQ = YUEQ.YUEQ;

    catch
        YL = zeros(total_msg_symbols, no_of_frames);
        YH = zeros(total_msg_symbols, no_of_frames);
        YEQ = zeros(total_msg_symbols, no_of_frames);
        YUEQ = zeros(total_msg_symbols, no_of_frames);
    end
    ...
    for n_detect = 1:length(st_id_list)
        ...
        if num == 1
            ...
            decoded_data = qamdemod(-detected_symbols_pc, mod_order, 'OutputType', 'bit', 'UnitAveragePower', true);
        ...
        if snr_estimate > 14
            ...
            YH(:, frame_num + 1) = abs(channel_symbols);
    ...
    save(strcat('Feedback_Files/Y', num2str(num), '_Output.mat'), 'YL');
    save(strcat('Feedback_Files/YH', num2str(num), '_Output.mat'), 'YH');
    save(strcat('Feedback_Files/YEQ', num2str(num), '_Output.mat'), 'YEQ');
    save(strcat('Feedback_Files/YUEQ', num2str(num), '_Output.mat'), 'YUEQ');
